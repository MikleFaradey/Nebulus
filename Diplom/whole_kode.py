...
def draw_cov_rectangle(ax, S, E, width, color="#6EC1FF", alpha=0.15, label=None):
    ...
    ax.add_patch(mpatches.Polygon([p1, p2, p3, p4], closed=True, facecolor=color, edgecolor="none", alpha=alpha, label=label))

class YawPD:
    def __init__(self, K1_kurs, K2_kurs, use_err_psi, use_err_r, meas_scale):
        self.K1 = float(K1_kurs)
        self.K2 = float(K2_kurs)
        self.use_err_psi = bool(use_err_psi)
        self.use_err_r = bool(use_err_r)
        self.meas_scale = float(meas_scale)

    def _meas_psi(self, psi):
        return self.meas_scale * psi if self.use_err_psi else psi

    def _meas_r(self, r):
        return self.meas_scale * r if self.use_err_r else r
...
class SurgePDRef:
    def __init__(self, Meff, C1, C2, K1_marsh, K2_marsh, wn, zeta, use_err_u, meas_scale, dt):
        self.Meff = float(Meff)
        self.C1 = float(C1)
        self.C2 = float(C2)
        self.K1 = max(0.0, float(K1_marsh))
        self.K2 = max(0.0, float(K2_marsh))
        self.wn = max(1e-6, float(wn))
        self.zeta = max(0.2, float(zeta))
        self.use_err_u = bool(use_err_u)
        self.meas_scale = float(meas_scale)
        self.dt = float(dt)
        self.ur = 0.0
        self.urd = 0.0
    ...
    def _meas_u(self, u):
        return self.meas_scale * u if self.use_err_u else u
...
class AUV_BFS_DRC:
    def __init__(self, p: Dict[str, Any]):
        ...
        self.Mdist = p["Mdist"]
        self.Kdx_thr = p["Kdx_thr"]
        self.Tdx_thr = p["Tdx_thr"]
        self.Umax_thr = abs(p["Umax_thr"])
        self.mix_surge = p["mix_gain_surge"]
        self.mix_yaw = p["mix_gain_yaw"]
        self.yaw_ctrl = YawPD(p["K1_kurs"], p["K2_kurs"], p["use_5pct_error_psi"], p["use_5pct_error_r"], p["meas_scale"])
        self.surge_ctrl = SurgePDRef(self.Meff, self.Cx1, self.Cx2, p["K1_marsh"], p["K2_marsh"], p["wn_ref"], p["zeta_ref"], p["use_5pct_error_u"], p["meas_scale"], self.dt)
        self.psi = 0.0
        self.r = 0.0
        self.u = 0.0
        self.x = 0.0
    ...
    def _drag_u(self, u):
        return self.Cx1 * abs(u) * u + self.Cx2 * u
    ...
    def step(self, u_ref_ground, psi_ref_deg):
        ...
        return UL, UR, M_thr, self._last_M_net
...
class SimResult:
    ...
    TL_hist: List[float]
    TR_hist: List[float]
    Mthr_hist: List[float]
    Mnet_hist: List[float]
    meta: Dict[str, Any]
...
def _estimate_Ts(p: Dict[str, Any]) -> Tuple[float, float]:
    ...
    while tloc < 8.0:
        ...
        if not reached and err <= 0.02:
            reached = True
            Ts_yaw = tloc
        if reached and err > 0.02:
            reached = False
            Ts_yaw = 8.0
    test = AUV_BFS_DRC(p)
    tloc = 0.0
...
def _auto_tune_for_2s(p: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if Ts_surge > 2.0:
        ...
        p["K2_marsh"] = p["K2_marsh"] * scale_surge if "K2_marsh" in p else p["K2_marsh"]
...
def run_simulation(p_in: Dict[str, Any]) -> SimResult:
    ...
    Ts_yaw, Ts_surge = _estimate_Ts(p)
    turn_buffer = 1.2 * U_cruise * max(Ts_yaw, Ts_surge)
    guard_before = float(p.get("guard_before_turn_m", 10.0))
    guard_after = float(p.get("guard_after_turn_m", 8.0))
    ...
    for S, E in acq_spans:
        ...
        spans_ex.append({"S": S, "E": E, "u": (ux, uy), "L": L, "psi_span": psi_span, "start_fired": False, "end_fired": False})
    auv = AUV_BFS_DRC(p)
    x0, y0 = wps[0]
    if len(wps) >= 2:
    ...
    EVENT_PERP_TOL = float(p.get("event_perp_tol", 5.0))
    EVENT_POINT_TOL = float(p.get("event_point_tol", 3.0))
    ...
    while t < T_end and i_seg < len(wps) - 1 and steps < max_steps:
        ...
        for sp in spans_ex:
            ...
            if (not sp["start_fired"]) and is_straight and (((s_prev < 0.0) and (s >= 0.0) and (d_perp <= EVENT_PERP_TOL)) or (dS <= EVENT_POINT_TOL)):
                _ = send_sonar_cmd("SSS_START", bool(p.get("sonar_serial_enable", False)), str(p.get("sonar_port", "/dev/ttyUSB0")), int(p.get("sonar_baud", 9600)))
                camera_events.append(("SSS_START", (Sx, Sy), t))
                sp["start_fired"] = True
            if (not sp["end_fired"]) and is_straight and (((s_prev < L) and (s >= L) and (d_perp <= EVENT_PERP_TOL)) or (dE <= EVENT_POINT_TOL)):
                _ = send_sonar_cmd("SSS_END", bool(p.get("sonar_serial_enable", False)), str(p.get("sonar_port", "/dev/ttyUSB0")), int(p.get("sonar_baud", 9600)))
        ...
        if capturing:
            ...
            u_ref_base = p["U_cruise"] * max(0.3, min(1.0, dist_to_end / (2.0 * p["lookahead"])))
        ...
        uL_hist.append(UL)
        uR_hist.append(UR)
        TL_hist.append(auv.TL)
        TR_hist.append(auv.TR)
        Mthr_hist.append(M_thr)
        Mnet_hist.append(M_net)
        if math.hypot(wp_j[0] - auv.x, wp_j[1] - auv.y) < p["switch_R"]:
            i_seg += 1
    meta = dict(W_use=W_use, lane_spacing=lane_spacing, s_ping=s_ping, dt_ping=dt_ping, turn_buffer=turn_buffer, U_acq=p["U_acq"], U_cruise=p["U_cruise"], ts_yaw=Ts_yaw, ts_surge=Ts_surge, autotune=p.get("auto_tune_2s", False), rect_orig=rect_orig)
    return SimResult(xs, ys, wps, rect, acq_spans, camera_events, t_hist, psi_deg, u_ground, e_ct_hist, uL_hist, uR_hist, TL_hist, TR_hist, Mthr_hist, Mnet_hist, meta)
...
class StepResponses:
    ...
    t_u_body: List[float]
    u_body: List[float]
    u_ref_body: List[float]
    u_body_band: Tuple[float, float]
    u_body_Mp_pct: float
    u_body_Ts: float | None
    t_x: List[float]
    x_coord: List[float]
    x_ref: List[float]
...
def compute_step_responses(p_in: Dict[str, Any]) -> StepResponses:
    ...
    dt = p["dt"]
    auv = AUV_BFS_DRC(p)
    T = max(dt, float(p["yaw_step_T"]))
    amp = float(p["yaw_step_amp_deg"])
    t_yaw = [0.0]
    yaw = [auv.psi * RAD2DEG]
    yref = [amp]
    tcur = 0.0
    ...
    while tcur < T:
        ...
        u_b.append(auv.u)
        u_ref_b.append(amp_ub)
        t_ub.append(tcur)
    u_b_band, u_b_Mp, u_b_Ts = _step_metrics(t_ub, u_b, u_ref_b, tol=0.05)
    Kp_x = float(p.get("Kp_x", 0.9))
    Kd_x = float(p.get("Kd_x", 0.6))
    u_max_pos = float(p.get("u_max_pos", 2.0))
    x_amp = float(p.get("x_step_amp", 1.0))
    Tpos = float(p.get("x_step_T", 12.0))
    auv = AUV_BFS_DRC(p)
    t_x = [0.0]
    xs = [auv.x]
    xref = [x_amp]
    ...
    return StepResponses(t_yaw, yaw, yref, yaw_band, yaw_Mp, yaw_Ts, t_ub, u_b, u_ref_b, u_b_band, u_b_Mp, u_b_Ts, t_x, xs, xref, x_band, x_Mp, x_Ts)
...
class ParamTable(QtWidgets.QTableWidget):
    ...
    def __init__(self, rows: List[ParamRow], parent=None):
        ...
        self._rows = rows
        self.spec_map: Dict[str, ParamRow] = {}
        self.populate(rows)
    ...
    def populate(self, rows: List[ParamRow]):
        ...
        for i, r in enumerate(rows):
            cat = QtWidgets.QTableWidgetItem(r.category)
            cat.setFlags(QtCore.Qt.ItemFlag.ItemIsEnabled)
            key = QtWidgets.QTableWidgetItem(r.label)
            key.setFlags(QtCore.Qt.ItemFlag.ItemIsEnabled)
            if r.ptype == "bool":
                ...
                val = QtWidgets.QTableWidgetItem(str(r.value))
                val.setFlags(QtCore.Qt.ItemFlag.ItemIsEditable | QtCore.Qt.ItemFlag.ItemIsEnabled | QtCore.Qt.ItemFlag.ItemIsSelectable)
            unit = QtWidgets.QTableWidgetItem(r.unit)
            unit.setFlags(QtCore.Qt.ItemFlag.ItemIsEnabled)
            desc = QtWidgets.QTableWidgetItem(r.desc)
            desc.setFlags(QtCore.Qt.ItemFlag.ItemIsEnabled)
    ...
    def load_from_dict(self, d: Dict[str, Any]):
        legacy = {"K1": "K1_kurs", "K2": "K2_kurs", "Kp": "K1_marsh", "Kd": "K2_marsh", "Kp_marsh": "K1_marsh", "Kd_marsh": "K2_marsh"}
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        top_layout.addWidget(self.chk_legend)
    ...
    def on_run(self):
        ...
        try:
            ...
            QtWidgets.QMessageBox.critical(self, "Ошибка симуляции", str(e))
            return
        try:
            steps = compute_step_responses(params)
        except Exception as e:
            QtWidgets.QMessageBox.critical(self, "Ошибка переходных", str(e))
            return
        for tab in (self.tab_plan, self.tab_yaw, self.tab_speed, self.tab_ect, self.tab_step_yaw, self.tab_step_u_body, self.tab_step_x, self.tab_ul, self.tab_tl, self.tab_mom):
            tab.fig.clf()
            tab.ax = tab.fig.add_subplot(111)
            tab.ax.grid(True)
        ax = self.tab_plan.ax
        xmin, ymin, W, H = res.rect
        ax.plot([xmin, xmin + W, xmin + W, xmin, xmin], [ymin, ymin, ymin + H, ymin + H, ymin], linestyle="-", color="k", linewidth=1.6, label=("Сектор (+10%)" if self.chk_legend.isChecked() else None))
        if "rect_orig" in res.meta:
            x0, y0, W0, H0 = res.meta["rect_orig"]
            ax.plot([x0, x0 + W0, x0 + W0, x0, x0], [y0, y0, y0 + H0, y0 + H0, y0], linestyle="--", color="gray", linewidth=1.2, label=("Сектор (заданный)" if self.chk_legend.isChecked() else None))
        if len(res.wps) >= 2:
            wx, wy = zip(*res.wps)
        ...
        for ev, P, _tt in res.camera_events:
            if ev == "SSS_START":
                self.tab_plan.ax.scatter(P[0], P[1], s=220, marker="*", zorder=12, facecolors="yellow", edgecolors="black", linewidths=1.2, label=("SSS_START" if self.chk_legend.isChecked() and not added_start else None))
                added_start = True
            elif ev == "SSS_END":
                self.tab_plan.ax.scatter(P[0], P[1], s=200, marker="P", zorder=12, facecolors="crimson", edgecolors="black", linewidths=1.2, label=("SSS_END" if self.chk_legend.isChecked() and not added_end else None))
                added_end = True
        ax.scatter(res.wps[0][0], res.wps[0][1], s=200, color="red", marker="o", edgecolors="black", linewidths=1.2, zorder=11, label=("Старт ПА" if self.chk_legend.isChecked() else None))
        ax.scatter(res.wps[-1][0], res.wps[-1][1], s=200, color="red", marker="X", edgecolors="black", linewidths=1.2, zorder=11, label=("Финиш ПА" if self.chk_legend.isChecked() else None))
        ax.set_aspect("equal", adjustable="box")
        ax.set_xlabel("x, м")
        ax.set_ylabel("y, м")
        autotune_txt = " | auto:ON" if res.meta.get("autotune", False) else ""
        ax.set_title(f"SSS: W≈{res.meta['W_use']:.1f} м | lane≈{res.meta['lane_spacing']:.1f} м | U_acq={res.meta['U_acq']:.1f} м/с | buffer≈{res.meta['turn_buffer']:.1f} м | t_s(yaw)≈{res.meta['ts_yaw']:.2f}s | t_s(surge)≈{res.meta['ts_surge']:.2f}s{autotune_txt}")
        ...
        ax.set_xlabel("t, c")
        ax.set_ylabel("|V_ground|, м/с")
        ax.set_title("Скорость (ground) v(t)")
        self.tab_speed.canvas.draw()
        ax = self.tab_ect.ax
        ax.plot(res.t_hist, res.e_ct_hist)
        ax.set_xlabel("t, c")
        ax.set_ylabel("e_ct, м")
        ax.set_title("Поперечная ошибка e_ct(t)")
        ...
        if steps.yaw_Ts is not None:
            ax.axvline(steps.yaw_Ts, linestyle=":", linewidth=1, label=f"Ts≈{steps.yaw_Ts:.2f}s")
        ...
        if steps.u_body_Ts is not None:
            ax.axvline(steps.u_body_Ts, linestyle=":", linewidth=1, label=f"Ts≈{steps.u_body_Ts:.2f}s")
        ax.set_title("Переходная по линейной скорости u (тело)")
        ax.set_xlabel("t, c")
        ax.set_ylabel("м/с")
        if self.chk_legend.isChecked():
        ...
        if steps.x_Ts is not None:
            ax.axvline(steps.x_Ts, linestyle=":", linewidth=1, label=f"Ts≈{steps.x_Ts:.2f}s")
        ax.set_title("Переходная по координате X (наружный ПД→u_ref)")
        ax.set_xlabel("t, c")
        ax.set_ylabel("м")
        if self.chk_legend.isChecked():
            yr = steps.x_ref[-1]
        ...
        ax.set_xlabel("t, c")
        ax.set_ylabel("В")
        ax.set_title("U_L/U_R (входы движителей)")
        if self.chk_legend.isChecked():
            ax.legend(loc="best")
        ...
        self.statusBar().showMessage(f"L≈{total_L:.1f} м; W≈{res.meta['W_use']:.1f} м; lane≈{res.meta['lane_spacing']:.1f} м; s_ping≈{res.meta['s_ping']:.2f} м; dt_ping≈{res.meta['dt_ping']:.2f} c; SSS_START={starts}, SSS_END={ends}")
